...
//		productDTO.setQandAs(productBasic.getQandA());
	...
	public Page<ProductBasicDto> getProductByPage2(Pageable pageable, String productname, Integer memberIdd) {
		if (memberIdd != null) {
			...
			Page<ProductBasic> pageByMemberId = productBasicRepository
					.findProductBasicBySellermemberidAndProductnameContaining(memberIdd, productname, pageable);

			List<ProductBasicDto> productDtos = pageByMemberId.getContent().stream().map(this::convertToProductBasicDto)
	...
	// 價格範圍搜尋
	@Transactional
	public Page<ProductCategoryDto> getCategoryNameByPriceRange(String categoryname, Double minPrice, Double maxPrice,
			Pageable pageable) {
		// 檢查價格範圍的合理性 // 價格不能為負數，處理相應的錯誤邏輯，最小價格不能大於最大價格，處理相應的錯誤邏輯
		if (minPrice < 0 || maxPrice < 0 || minPrice > maxPrice) {
			// 如果價格範圍無效，返回包含錯誤訊息的結果
		...
		Page<ProductBasic> productBasics = productCategoryRepository
				.findProductBasicsByCategoryIdAndPriceRange(categoryid, minPrice, maxPrice, pageable);
...
//                    }
	...
	public ProductBasic updateProduct(ProductBasic ed, ProductBasic up) {
		...
		if (up.getCategoryid() != null) {
			...
			System.out.println("Parentid" + up.getCategoryid().getParentid());
			Integer categoryid = up.getCategoryid().getId();
			int parentid = productCategoryRepository.findParentIdbyCategoryid(categoryid);
			Optional<ParentCategory> parentCategoryOptional = parentCategoryRepository.findById(parentid);
			ParentCategory parentCategory = parentCategoryOptional.get();
			System.out.println(parentid);

			ed.setParentid(parentCategory);
		}
	...
	public Page<ProductBasicDto> getAllProductByPage(Pageable pageable, Integer memberIdd) {
		if (memberIdd != null) {
			Page<ProductBasic> pageByMemberId = productBasicRepository.findProductBasicBySellermemberid(memberIdd,
					pageable);
